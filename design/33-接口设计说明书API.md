...
| 12xxx 审批 | 12001 工单不存在（404）；12002 工单已处理（409）；12003 工单已过期（409）；12004 回调签名校验失败（401）；12005 审批人无权处理（403）；12006 操作已被 HITL 审批拦截（403，即 BLOCKED_BY_HITL）；12007 审批流未配置（503）；12008 审批渠道推送失败（503） | |
...
```json
{
  ...
  "is_error": true,
  "execution_duration_ms": 402
}
```

- 错误码：10002、10003（Key 权限范围不含该工具，FR-009 验收）、11001、11002（设备离线）、11003（设备冻结）、11005、11006、11007（调用超时）、12007（高危工具但审批流未配置，fail-safe 直接拒绝）、13003（租户调用量配额超限）、10006。
...
- 错误码：12004（签名校验失败）、12001（工单不存在）、12002（工单已处理，重复回调）、12003（工单已过期）、12005（审批人不在该工单审批人名单）、10001（decision 非 approve/reject）。
